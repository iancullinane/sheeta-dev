...
export class Network extends Construct {
  ...
  constructor(scope: Construct, id: string, props: NetworkProps) {
    ...
    for (const key in props.cfg.networks) {
      ...
      if (temp.new) {
        var newHz = new r53.HostedZone(this, `network-${key}`, {
          zoneName: key,
        });
      } else if (temp.hostedZoneId) {
        var hz = r53.HostedZone.fromLookup(this, `tld-hz-lookup-${key}`, {
          domainName: key,
        });

        const cert = new acm.Certificate(this, `Certificate-${key}`, {
          domainName: key,
